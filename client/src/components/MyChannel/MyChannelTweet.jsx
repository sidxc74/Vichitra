...
import { getTweetApi } from '../../api/tweet.api'
import { getTimeDifference } from '../../utils/timeDifference'

function MyChannelTweet({user}) {
    ...
    const { data : tweets } = useQuery({
        queryKey: ['tweets'],
        queryFn : async () => {
            const res = await getTweetApi(user._id)
  ...
  return (
    <>
        {
            tweets?.map((tweet) => (
          <div key={tweet._id} class="flex gap-3 border-b border-gray-700 py-4 last:border-b-transparent">
            ...
            <div class="w-full">
              <h4 class="mb-1 flex items-center gap-x-2">
                ...
                 
                <span class="inline-block text-sm text-gray-400">{getTimeDifference(tweet?.createdAt || 0)}</span>
              </h4>
              ...
              <div class="flex gap-4">
                <button
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    aria-hidden="true"
                    class="h-5 w-5 text-red-600 group-focus:text-inherit">
                    <path
                ...
                <button
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    aria-hidden="true"
                    class="h-5 w-5 text-inherit group-focus:text-red-600">
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
